...
TaskHistory.find_by_sql("select count(*) total, completed_on from task_histories LEFT JOIN tasks ON task_histories.task_id=tasks.id WHERE tasks.user_id=#{user_id} and task_histories.completed_on BETWEEN '#{start_of_week}' and '#{end_of_week}' group by completed_on ").collect{ |t| t.total.to_i}
  end
